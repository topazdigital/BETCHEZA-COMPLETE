...
const POLL_INTERVAL_IDLE   = 120_000  // 2min outside match window

function isMatchWindowActive(kickoffFrom?: string | null, kickoffTo?: string | null): boolean {
  if (!kickoffFrom || !kickoffTo) return true // always poll if no window set
  const now = Date.now()
  const from = new Date(kickoffFrom).getTime() - 30 * 60 * 1000 // 30min before kickoff
  const to   = new Date(kickoffTo).getTime()   + 3  * 60 * 60 * 1000 // 3h after last kickoff
