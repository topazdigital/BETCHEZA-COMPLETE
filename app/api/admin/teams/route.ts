...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sportId && sportId !== 'all') { conditions.push('t.sport_id = ?'); params.push(parseInt(sportId)); }
    if (leagueId && leagueId !== 'all') { conditions.push('t.league_id = ?'); params.push(parseInt(leagueId)); }
    if (search) { conditions.push('t.name LIKE ?'); params.push(`%${search}%`); }

    const whereClause = conditions.join(' AND ');

    const result = await query(`
      SELECT t.id, t.name, t.slug, t.logo_url, t.api_id,
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM teams t WHERE ${whereClause}`, params);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !sportId || !countryId) {
      return NextResponse.json({ error: 'Missing required fields: name, sportId, countryId' }, { status: 400 });
    }
