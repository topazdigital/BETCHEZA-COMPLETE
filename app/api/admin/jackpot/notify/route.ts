...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (result.skipped) return NextResponse.json({ ok: false, message: 'Email not sent — SMTP is not configured. Go to Admin Settings to set it up.', sent: 0, failed: 0, recipientCount: result.recipientCount });
    if (result.recipientCount === 0) return NextResponse.json({ ok: true, message: 'No subscribers have opted in to jackpot alerts yet.', sent: 0, failed: 0, recipientCount: 0 });
    return NextResponse.json({ ok: true, message: 'Sent to ' + result.sent + ' subscriber' + (result.sent !== 1 ? 's' : '') + (result.failed > 0 ? ' (' + result.failed + ' failed)' : '') + '.', sent: result.sent, failed: result.failed, recipientCount: result.recipientCount });
