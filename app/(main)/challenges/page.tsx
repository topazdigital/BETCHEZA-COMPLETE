...
function TeamLogo({ src, name, size = 28 }: { src: string | null; name: string; size?: number }) {
  ...
  if (!src || err) {
    return (
      <div className="rounded-full bg-gray-700 flex items-center justify-center font-bold text-white shrink-0"
...
function ParticipantAvatar({ avatar, name, size = 28 }: { avatar: string | null; name: string; size?: number }) {
  ...
  if (!avatar || err) {
    return (
      <div className="rounded-full bg-gradient-to-br from-blue-600 to-purple-700 flex items-center justify-center font-bold text-white shrink-0"
...
function PickBadge({ pick, won, draw }: { pick: string; won?: boolean; draw?: boolean }) {
  const cls = won ? 'bg-green-600 text-white' : draw ? 'bg-yellow-600 text-black' : 'bg-gray-700 text-gray-200';
    ...
    <div ref={wrap} className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full max-h-72 overflow-y-auto rounded-xl border border-gray-700 bg-gray-900 shadow-2xl">
          ...
          {!loading && results.length === 0 && <div className="p-3 text-center text-gray-500 text-sm">No upcoming matches found</div>}
          {results.map(m => (
            <button key={m.id} onClick={() => { onSelect(m); setQ(`${m.homeTeam} vs ${m.awayTeam}`); setOpen(false); }}
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-white truncate">{m.homeTeam} <span className="text-gray-500">vs</span> {m.awayTeam}</div>
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Pick a match · Choose your prediction · Set your stake</p>
          </div>
        ...
        <div className="p-5 space-y-5 overflow-y-auto flex-1">
          <div>
            <label className="block text-sm font-semibold text-gray-300 mb-2">1. Select Match</label>
            ...
            {selectedMatch && (
              <div className="mt-2 p-3 rounded-xl bg-gray-800 border border-gray-700">
                ...
                {selectedMatch.odds && (selectedMatch.odds.home > 0 || selectedMatch.odds.away > 0) && (
                  <div className="mt-3 pt-2.5 border-t border-gray-700 flex justify-center gap-5">
                    {selectedMatch.odds.home > 0 && (
                      <div className="text-center"><div className="text-xs text-gray-500">1 Home</div><div className="text-sm font-bold text-white">{selectedMatch.odds.home.toFixed(2)}</div></div>
                    )}
                    {selectedMatch.odds.draw > 0 && (
                      <div className="text-center"><div className="text-xs text-gray-500">X Draw</div><div className="text-sm font-bold text-white">{selectedMatch.odds.draw.toFixed(2)}</div></div>
                    )}
                    {selectedMatch.odds.away > 0 && (
                      <div className="text-center"><div className="text-xs text-gray-500">2 Away</div><div className="text-sm font-bold text-white">{selectedMatch.odds.away.toFixed(2)}</div></div>
                    )}
                  </div>
                )}
              </div>
                    ...
                    {opts.map(o => {
                      ...
                      return (
                        <button key={o.value} onClick={() => setPick(o.value)}
                          className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-all flex flex-col items-center leading-tight ${pick === o.value ? 'bg-blue-600 border-blue-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-300 hover:border-gray-500 hover:text-white'}`}>
                          <span>{o.label}</span>
                          {odd !== null && <span className={`text-xs font-bold mt-0.5 ${pick === o.value ? 'text-blue-200' : 'text-yellow-400'}`}>{odd.toFixed(2)}</span>}
                        </button>
                      );
                    })}
                  </div>
            ...
            <div>
              ...
              {stakeKes > 0 && (
                <p className="text-xs text-gray-400 mt-2">
                  Winner takes <span className="text-green-400 font-semibold">KES {Math.round(stakeKes * 2 * 0.9).toLocaleString()}</span>
                  <span className="text-gray-500"> · 10% fee · Draw = full refund</span>
                </p>
              )}
              <div className="flex items-center gap-3 mt-3 p-3 rounded-lg bg-gray-800/50 border border-gray-700/50">
                ...
                <div>
                  <div className="text-sm font-medium text-white">{isPublic ? 'Open — any tipster can accept' : 'Private — invite only'}</div>
                  <div className="text-xs text-gray-400">{isPublic ? 'Appears in the Open tab for everyone' : 'Only your opponent can join'}</div>
                </div>
              </div>
            </div>
          )}
          ...
          {error && (
            <div className="rounded-lg p-3 bg-red-900/30 border border-red-800 text-red-300 text-sm">
              {error}
              {insufficientBalance && <a href="/dashboard/wallet" className="block mt-1.5 text-blue-400 text-xs underline">Top up wallet →</a>}
            </div>
          )}
        </div>
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Counter-pick · May the best tipster win</p>
          </div>
        ...
        <div className="p-5 space-y-4">
          <div className="p-3 rounded-xl bg-gray-800 border border-gray-700">
            <div className="flex items-center justify-center gap-4">
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-gray-400">{challenge.matchLeague}</div>
          ...
          <div className="flex items-center gap-2.5 p-3 rounded-xl bg-gray-800/50 border border-gray-700">
            ...
            <div className="flex-1 text-sm">
              <div className="font-semibold text-white">{challenge.challenger?.displayName || `User #${challenge.challengerId}`}</div>
              <div className="text-gray-400 text-xs mt-0.5">picked <PickBadge pick={challenge.challengerPick} /></div>
            </div>
...
function WatchButton({ challengeId, initialWatchers }: { challengeId: number; initialWatchers: number }) {
  ...
  return (
    <button onClick={handleWatch} disabled={watching || busy}
      ...
      className={`flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-medium transition-colors border ${watching ? 'border-purple-700/50 bg-purple-900/20 text-purple-300 cursor-default' : 'border-gray-700 bg-gray-800/60 text-gray-400 hover:border-gray-500 hover:text-white'}`}>
      <span>👁</span>
...
function ChallengeCard({ challenge, currentUserId, onAccept, onCancel }: {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-all ${settled ? 'border-gray-800 bg-gray-900/40' : active ? 'border-green-800/50 bg-gray-900 shadow-lg shadow-green-900/10' : 'border-gray-700 bg-gray-900 hover:border-gray-600'}`}>
      {/* Match header */}
      <div className="px-4 pt-3 pb-2 border-b border-gray-800/60">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-xs text-gray-400 truncate flex-1 mr-2">{challenge.matchLeague}</span>
          <div className="flex items-center gap-1.5 shrink-0">
            {!settled && <WatchButton challengeId={challenge.id} initialWatchers={challenge.watchers} />}
            {settled && challenge.watchers > 0 && <span className="flex items-center gap-0.5 text-xs text-gray-600">👁 {challenge.watchers}</span>}
            {settled ? (
              isDraw ? <span className="px-2 py-0.5 rounded text-xs font-bold bg-yellow-700/30 text-yellow-300 border border-yellow-700/50">🤝 Draw</span>
      ...
      <div className="px-4 py-3">
        <div className="flex items-stretch gap-2">
          ...
          <div className={`flex-1 rounded-xl p-3 border ${challengerWon ? 'bg-green-900/20 border-green-700/50' : isDraw ? 'bg-yellow-900/10 border-yellow-700/30' : settled ? 'bg-gray-800/30 border-gray-700/30' : 'bg-gray-800/50 border-gray-700/50'}`}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500">{challenge.challenger?.won ?? 0}W / {challenge.challenger?.lost ?? 0}L</div>
              </div>
          ...
          <div className={`flex-1 rounded-xl p-3 border ${challengedWon ? 'bg-green-900/20 border-green-700/50' : isDraw ? 'bg-yellow-900/10 border-yellow-700/30' : settled ? 'bg-gray-800/30 border-gray-700/30' : challenge.challenged ? 'bg-gray-800/50 border-gray-700/50' : 'bg-gray-800/20 border-dashed border-gray-600'}`}>
            {challenge.challenged ? (
              <>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500">{challenge.challenged.won ?? 0}W / {challenge.challenged.lost ?? 0}L</div>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center justify-center h-full gap-1 text-center py-0.5">
                ...
                <div className="text-xs text-gray-500">Any tipster</div>
                {canAccept && (
        ...
        {settled && challenge.stakeKes > 0 && (
          <div className="mt-2 text-center text-xs">
            {isDraw ? (
              <span className="text-yellow-400">Stakes refunded · Draw on this market</span>
            ) : winnerId ? (
          ...
          <div className="mt-3 flex gap-2">
            ...
            {canCancel && (
              <button onClick={() => onCancel(challenge.id)}
                className="px-4 py-2 rounded-xl bg-gray-800 hover:bg-red-900/30 text-gray-400 hover:text-red-400 text-sm border border-gray-700 transition-colors">
                Cancel
              </button>
            )}
          </div>
...
function ChallengeLeaderboard({ challenges }: { challenges: Challenge[] }) {
  type Stats = { name: string; avatar: string | null; won: number; total: number; potKes: number };
  const stats: Record<number, Stats> = {};

  for (const c of challenges) {
    if (c.status !== 'settled') continue;

    const upsert = (id: number, p: Participant | null) => {
      if (!p) return;
      if (!stats[id]) stats[id] = { name: p.displayName, avatar: p.avatar, won: 0, total: 0, potKes: 0 };
      stats[id].total++;
      if (c.winnerId === id && !c.drawRefunded) {
        stats[id].won++;
        stats[id].potKes += Math.round(c.stakeKes * 2 * 0.9);
      }
    };
  ...
  const leaders = Object.entries(stats)
    .filter(([, s]) => s.total >= 1)
    .sort(([, a], [, b]) => {
      const aw = a.won / a.total;
      const bw = b.won / b.total;
      return bw !== aw ? bw - aw : b.won - a.won;
    })
    .slice(0, 8);
  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900 p-4">
      <div className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1.5">
        <span>🏆</span> Top Challengers
      </div>
      {leaders.length === 0 ? (
        <p className="text-xs text-gray-600 text-center py-3">No settled challenges yet — first battle wins the top spot!</p>
      ) : (
          ...
          {leaders.map(([id, s], i) => {
            ...
            return (
              <div key={id} className="flex items-center gap-2">
                <span className="w-5 text-center text-sm shrink-0">{MEDALS[i] || <span className="text-xs text-gray-600 font-bold">{i + 1}</span>}</span>
                <ParticipantAvatar avatar={s.avatar} name={s.name} size={26} />
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold text-white truncate">{s.name}</div>
                  <div className="text-xs text-gray-500">{s.won}W / {s.total - s.won}L</div>
                </div>
                <div className={`text-xs font-bold shrink-0 ${winPct >= 70 ? 'text-green-400' : winPct >= 50 ? 'text-yellow-400' : 'text-gray-400'}`}>
                  {winPct}%
                </div>
              </div>
            );
      ...
      {leaders.length > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-800">
          <p className="text-xs text-gray-600 text-center">Updated in real-time as challenges settle</p>
        </div>
      )}
    </div>
  );
...
function RecentActivity({ challenges }: { challenges: Challenge[] }) {
  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900 p-4">
      <div className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Recent Wins</div>
      <div className="space-y-2.5">
        {recent.map(c => {
          ...
          return (
            <div key={c.id} className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-semibold text-white truncate">{winner?.displayName || 'Tipster'}</div>
                <div className="text-xs text-gray-500 truncate">{c.matchHomeTeam} vs {c.matchAwayTeam}</div>
              </div>
              {payout > 0 && <div className="text-xs font-bold text-green-400 shrink-0">+{payout.toLocaleString()}</div>}
            </div>
          );
        })}
      </div>
...
export default function ChallengesPage() {
  ...
  // Auto-switch to Open tab when nothing is live
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-6">
            <div className="flex gap-1 p-1 bg-gray-900 rounded-xl border border-gray-800 mb-4">
              {STATUS_TABS.map(t => (
                <button key={t.value} onClick={() => setStatusTab(t.value)}
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${statusTab === t.value ? 'bg-gray-800 text-white shadow' : 'text-gray-400 hover:text-white'}`}>
                  {t.label}
                  {t.value === 'active' && stats.live > 0 && <span className="ml-1 px-1.5 py-0.5 bg-red-600 text-white text-xs rounded-full">{stats.live}</span>}
                  {t.value === 'pending' && stats.open > 0 && <span className="ml-1 px-1.5 py-0.5 bg-blue-600 text-white text-xs rounded-full">{stats.open}</span>}
                </button>
              ))}
            </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {!user && (
              <div className="rounded-xl border border-blue-800/50 bg-blue-900/10 p-4 text-center">
                <div className="text-2xl mb-2">⚔️</div>
                <div className="text-sm font-semibold text-white mb-1">Join the Arena</div>
                <p className="text-xs text-gray-400 mb-3">Sign up to post challenges and compete against other tipsters.</p>
                <a href="/register" className="block py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-colors">
                  Sign Up Free
                </a>
              </div>
            )}
