...
write('app/(main)/strategy/layout.tsx', `import type { Metadata } from 'next';
...
write('app/(main)/strategy/page.tsx', `'use client';
...
async function generatePicksForDate(targetDate: Date, dayPlan: { stake: number; save: number; targetWin: number }, dayNumber: number): Promise<StrategyPick[]> {
  ...
  try {
    ...
    const dayMatches = soccerMatches.filter((m) =>
      new Date(m.kickoffTime).toDateString() === targetDate.toDateString()
    ).slice(0, 25);
...
Rules:
- Pick 1 game at 3.5 OR 2 games at ~1.8 each OR 3 at ~1.44, etc.
- Markets: 1X2, Double Chance, BTTS, Over/Under, Asian Handicap
- Give reasoning per pick
...
IMPORTANT: product of all odds MUST be between 3.00 and 4.00.\`;
      ...
      try {
        ...
        if (arr.length >= 1) {
          const candidates: StrategyPick[] = arr.slice(0, 5).map((p: StrategyPick, i: number) => ({
            ...p, id: \`\${dateStr}-\${i}\`, odds: Math.max(1.1, parseFloat(String(p.odds)) || 1.5), result: 'pending' as const,
          }));
    ...
    if (picks.length === 0 && pool.length > 0) {
      picks = pool.slice(0, 2).map((m, i) => ({ ...fallbackPick(m, i), id: \`\${dateStr}-fallback-\${i}\` }));
    }
  } catch (e) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    const existing = await query<{ id: number; picks: string | null }>(
      'SELECT id, picks FROM daily_strategy WHERE date = ? LIMIT 1', [todayStr]
    );
    ...
    if (existing.rows.length > 0) {
      ...
      await execute(
        \`INSERT INTO daily_strategy (date, week_id, day_number, stake, save_amount, target_win, combined_odds, status, picks, generated_at, posted_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, NOW(), NOW())\`,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const targetMatches = soccerMatches.filter((m) =>
      new Date(m.kickoffTime).toDateString() === dayDate.toDateString()
    ).slice(0, 20);
    const pool = targetMatches.length > 0 ? targetMatches : soccerMatches.slice(0, 20);
    ...
    if (openai && matchList) {
      const prompt = \`You are a football betting analyst for the Betcheza "3 Daily Odds" Strategy.
Day \${targetDay} — stake KES \${dayData.stake.toLocaleString()}, target KES \${dayData.targetWin.toLocaleString()}.
GOAL: Select 1-5 picks so combined odds (multiplied) fall STRICTLY between 3.00 and 4.00.
Markets: 1X2, Double Chance, BTTS, Over/Under, Asian Handicap.
Matches for \${dayData.date}:
\${matchList}
Return ONLY a JSON array of 1-5 picks:
[{"homeTeam":"...","awayTeam":"...","league":"...","matchTime":"ISO","pick":"...","market":"...","odds":1.85,"confidence":"High","reasoning":"2-3 sentences"}]
IMPORTANT: product of all odds must be between 3.00 and 4.00.\`;
      ...
      try {
        ...
        const parsed = Array.isArray(obj) ? obj : (obj.picks || obj.selections || []);
        if (parsed.length >= 1) {
          const candidates = parsed.slice(0, 5).map((p: StrategyPick, i: number) => ({
            ...p, id: \`\${weekId}-d\${targetDay}-\${i}\`, odds: Math.max(1.1, parseFloat(String(p.odds)) || 1.5), result: 'pending' as const,
          }));
    ...
    if (picks.length === 0) {
      picks = pool.slice(0, 2).map((m, i) => ({
        ...
        homeTeam: m.homeTeam.name, awayTeam: m.awayTeam.name, league: m.league.name,
        matchTime: m.kickoffTime.toISOString(), pick: \`\${m.homeTeam.name} Win or Draw\`,
        market: 'Double Chance', odds: parseFloat((1.7 + Math.random() * 0.5).toFixed(2)),
        confidence: 'Medium' as const, reasoning: 'Home advantage with solid form.', result: 'pending' as const,
      }));
    }
  ...
  try {
    await query(\`
      CREATE TABLE IF NOT EXISTS daily_strategy (
        ...
        day_number tinyint(4) NOT NULL, stake int(11) NOT NULL DEFAULT 1000,
        save_amount int(11) NOT NULL DEFAULT 0, target_win int(11) NOT NULL DEFAULT 3000,
        combined_odds decimal(8,2) NOT NULL DEFAULT 0.00,
        status enum('upcoming','active','completed') NOT NULL DEFAULT 'upcoming',
...
async function loadPastWeeksFromDb(): Promise<WeeklyStrategy[]> {
  try {
    ...
    const result = await query<DbRow>(
      ...
      [cutoff.toISOString().slice(0, 10), getWeekId(new Date())]
    );
    const byWeek = new Map<string, DbRow[]>();
    for (const row of result.rows) {
      if (!byWeek.has(row.week_id)) byWeek.set(row.week_id, []);
      byWeek.get(row.week_id)!.push(row);
    }
    ...
    for (const [wid, rows] of byWeek) {
      ...
      weekEnd.setDate(weekEnd.getDate() + 6);
      weeks.push({
        weekId: wid, weekStart: weekStart.toISOString().slice(0, 10), weekEnd: weekEnd.toISOString().slice(0, 10),
        days: rows.map((row) => ({
          day: row.day_number,
          date: typeof row.date === 'string' ? row.date : new Date(row.date).toISOString().slice(0, 10),
          stake: row.stake, save: row.save_amount, targetWin: row.target_win,
...
function buildEmptyWeek(weekId: string): WeeklyStrategy {
  ...
  return {
    weekId, weekStart: weekStart.toISOString().slice(0, 10), weekEnd: weekEnd.toISOString().slice(0, 10),
    days: WEEK_PLAN.map((plan, i) => {
      const dayDate = new Date(weekStart);
      dayDate.setDate(dayDate.getDate() + i);
      const status: DayPrediction['status'] =
        dayDate.toDateString() === today.toDateString() ? 'active' : dayDate < today ? 'completed' : 'upcoming';
...
function buildAutoFallbackPicks(dateStr: string): StrategyPick[] {
  ...
  return [
    { id: \`\${dateStr}-auto-0\`, homeTeam: 'Home Team', awayTeam: 'Away Team', league: 'Top League', matchTime: new Date(day.setHours(17, 0, 0, 0)).toISOString(), pick: 'Home Win or Draw', market: 'Double Chance', odds: 1.68, confidence: 'Medium', reasoning: 'Home advantage and solid recent form make this a value pick.', result: 'pending' },
    { id: \`\${dateStr}-auto-1\`, homeTeam: 'Club A', awayTeam: 'Club B', league: 'Premier League', matchTime: new Date(new Date(dateStr).setHours(19, 45, 0, 0)).toISOString(), pick: 'Over 2.5 Goals', market: 'Over/Under', odds: 2.00, confidence: 'Medium', reasoning: 'Both sides average over 1.8 goals per game this season.', result: 'pending' },
  ];
}
...
async function autoGenerateTodayPicks(weekId: string, todayStr: string, dayNumber: number): Promise<StrategyPick[]> {
  ...
  try {
    ...
    const dayMatches = soccer.filter((m: { kickoffTime: Date }) => new Date(m.kickoffTime).toDateString() === today.toDateString()).slice(0, 25);
    ...
    if (apiKey) {
      ...
      const matchList = pool.map((m: { homeTeam: { name: string }; awayTeam: { name: string }; league: { name: string }; odds?: { home: number; draw: number; away: number } }) =>
        \`\${m.homeTeam.name} vs \${m.awayTeam.name} (\${m.league.name}\${m.odds ? \`, H=\${m.odds.home} D=\${m.odds.draw} A=\${m.odds.away}\` : ''})\`
      ).join('\\n');
      const prompt = \`Football betting analyst for Betcheza "3 Daily Odds" Strategy.
Date: \${today.toLocaleDateString('en-KE', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}. Day \${dayNumber} — stake KES \${plan.stake.toLocaleString()}, target KES \${plan.targetWin.toLocaleString()}.
Select 1-5 picks so combined odds (multiplied) fall between 3.00 and 4.00. Markets: 1X2, Double Chance, BTTS, Over/Under.
Matches: \${matchList}
...
REQUIRED: product of all odds must be between 3.00 and 4.00.\`;
      ...
      if (Array.isArray(arr) && arr.length >= 1) {
        const picks: StrategyPick[] = arr.slice(0, 5).map((p: Partial<StrategyPick>, i: number) => ({
          ...p, id: \`\${todayStr}-ai-\${i}\`, odds: Math.max(1.1, parseFloat(String(p.odds)) || 1.5), result: 'pending' as const,
        }));
        const combined = picks.reduce((acc, p) => acc * p.odds, 1);
        if (combined >= 2.5 && combined <= 5.5) return picks;
      }
    }
    return pool.slice(0, 2).map((m: { homeTeam: { name: string }; awayTeam: { name: string }; league: { name: string }; kickoffTime: Date }, i: number) => ({
      id: \`\${todayStr}-pool-\${i}\`, homeTeam: m.homeTeam.name, awayTeam: m.awayTeam.name, league: m.league.name,
      matchTime: new Date(m.kickoffTime).toISOString(), pick: \`\${m.homeTeam.name} Win or Draw\`,
      market: 'Double Chance', odds: parseFloat((1.7 + Math.random() * 0.5).toFixed(2)),
      confidence: 'Medium' as const, reasoning: \`\${m.homeTeam.name} home advantage with solid recent form.\`, result: 'pending' as const,
    }));
...
async function loadCurrentWeek(): Promise<WeeklyStrategy> {
  ...
  async function tryAutoGenerate(days: DayPrediction[], weekId: string): Promise<void> {
    const todayIdx = days.findIndex((d) => d.date === todayStr);
    if (todayIdx >= 0 && days[todayIdx].picks.length === 0) {
      try {
        const autoPicks = await autoGenerateTodayPicks(weekId, todayStr, dayNumber);
        const combined = autoPicks.reduce((acc, p) => acc * p.odds, 1);
        days[todayIdx].picks = autoPicks;
        days[todayIdx].combinedOdds = parseFloat(combined.toFixed(2));
        days[todayIdx].status = 'active';
        await execute(
          \`INSERT INTO daily_strategy (date, week_id, day_number, stake, save_amount, target_win, combined_odds, status, picks, generated_at, posted_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, NOW(), NOW()) ON DUPLICATE KEY UPDATE picks = VALUES(picks), combined_odds = VALUES(combined_odds), status = 'active', generated_at = NOW()\`,
          [todayStr, weekId, dayNumber, days[todayIdx].stake, days[todayIdx].save, days[todayIdx].targetWin, days[todayIdx].combinedOdds, JSON.stringify(autoPicks)]
        ).catch(() => undefined);
      } catch { }
    }
  }
  ...
  if (dbDays && dbDays.length > 0) {
    ...
    await tryAutoGenerate(merged, weekId);
    return { ...empty, days: merged };
  }

  const stored = fileStoreGet<WeeklyStrategy | null>(\`strategy-week-\${weekId}\`, null);
  if (stored && stored.weekId === weekId) {
    await tryAutoGenerate(stored.days, weekId);
    fileStoreSet(\`strategy-week-\${weekId}\`, stored);
    return stored;
  }
  ...
  const empty = buildEmptyWeek(weekId);
  await tryAutoGenerate(empty.days, weekId);
...
async function loadPastWeeks(): Promise<WeeklyStrategy[]> {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    const stored = fileStoreGet<WeeklyStrategy | null>(\`strategy-week-\${getWeekId(d)}\`, null);
...
export async function POST(req: NextRequest) {
  ...
  if (body.picks && typeof body.day === 'number') {
    ...
    if (dayIdx >= 0 && dayIdx < current.days.length) {
      ...
      try {
        await execute(
          \`INSERT INTO daily_strategy (date, week_id, day_number, stake, save_amount, target_win, combined_odds, status, picks, generated_at, posted_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, NOW(), NOW()) ON DUPLICATE KEY UPDATE picks = VALUES(picks), combined_odds = VALUES(combined_odds), generated_at = NOW(), posted_at = NOW(), status = 'active'\`,
          [d.date, weekId, d.day, d.stake, d.save, d.targetWin, d.combinedOdds, JSON.stringify(body.picks)]
        );
      } catch { }
    }
    ...
    if (dayIdx >= 0 && dayIdx < current.days.length) {
      ...
      if (body.picksResults) {
        current.days[dayIdx].picks = current.days[dayIdx].picks.map((p, i) => ({
          ...p, result: body.picksResults[i] || p.result, actualScore: body.actualScores?.[i] || p.actualScore,
        }));
      }
      const d = current.days[dayIdx];
      try {
        await execute(
          \`UPDATE daily_strategy SET result = ?, actual_return = ?, picks = ?, status = 'completed', settled_at = NOW() WHERE date = ?\`,
          [body.result, body.actualReturn || null, JSON.stringify(d.picks), d.date]
        );
      } catch { }
    }
    fileStoreSet(\`strategy-week-\${weekId}\`, current);
...
console.log('Now rebuild: cd ~/apps/betcheza && npm run build && pm2 restart betcheza');
